...
class RViwerAdapter(listViewer:ArrayList<String>,val listener: RViwerAdapter.MyOnClickListener):RecyclerView.Adapter<RViwerAdapter.RVHolder>() {
    val listViewer = listViewer

    class RVHolder(item: View):RecyclerView.ViewHolder(item) {
        val binding = MemberItemBinding.bind(item)
        fun bind(viewer:String,listener: RViwerAdapter.MyOnClickListener) = with(binding){
